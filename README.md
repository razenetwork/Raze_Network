...
| ------------- | ------------- | ------------- |
| 0a. | License | Apache 2.0 / MIT / Unlicense |
| 1. | Raze client module | We will implement the client that supports the Register, CreateMintTx, CreateTransferTx, CreateRedeemTx, CreateLockTx, and CreateUnlockTx algorithms. This module will allow the client side to generate the necessary transactions to trigger the corresponding modules. 
| 2. | Privacy-preserving DeFi functionality | Through combining the lock and mint, and unlock and redeem modules, we will implement anonymous mining functionality, which allows the users to mine the private tokens and unlock the private tokens after a certain period of time.   
